...
class Recommender:
    ...
    @staticmethod
    async def check_history_async(user, db: AsyncSession):
        result = await db.execute(select(exists().where(UserHistory.user_id == user.id)))
        # result.scalar() returns a boolean for EXISTS in SQLAlchemy 2.0
    ...
    @staticmethod
    async def get_weighted_embedding_and_ids_async(user, db: AsyncSession, hist_len=10, decay_strength=1.0):
        stmt = (
    ...
    @staticmethod
    async def personalized_feed_async(user_id, weighted_embedding, article_ids, db: AsyncSession, page=1, page_size=10, recency_factor=0.3):
        offset = (page - 1) * page_size
        current_time = datetime.now()
        combined_score = (Articles.embeddings.cosine_distance(weighted_embedding) * (1 - recency_factor) +
                          func.extract('epoch', current_time - Articles.published_date) / 86400.0 * recency_factor
                          ).label("combined_score")

        stmt = build_article_select(user_id, combined_score)
        stmt = stmt.where(~Articles.id.in_(article_ids)).order_by(combined_score)
    ...
    @staticmethod
    async def get_recommendations_async(user, db: AsyncSession, page=1, page_size=20, recency_factor=0.3, decay_strength=1.0):
        hist_exist = await Recommender.check_history_async(user, db)
        if not hist_exist:
            return []

        weighted_embedding, article_ids = await Recommender.get_weighted_embedding_and_ids_async(
            user, db, hist_len=10, decay_strength=decay_strength)

        return await Recommender.personalized_feed_async(
            user.id, weighted_embedding, article_ids, db, page, page_size, recency_factor)
